<div x-data="{ open: {{ isset($open) && $open ? 'true' : 'false' }}, working: false }" x-cloak wire:key="delete-{{ $value }}">
    <span x-on:click="open = true">
        <button class="p-1 rounded text-danger hover:bg-danger hover:text-white">
            <x-icons.trash />
        </button>
    </span>
                        ...
                        <div class="flex justify-center mt-10">
                            ...
                            <span x-on:click="working = !working">
                                <button wire:click="delete({{ $value }})"
                                    class="inline-flex items-center justify-center w-32 px-3 py-2 text-sm font-medium leading-4 text-white transition duration-150 ease-in-out border border-transparent rounded-md shadow-sm bg-danger hover:bg-red-700 focus:outline-none focus:border-red-700 focus:shadow-outline-teal active:bg-red-700">
                                    {{ __('Yes') }}
                                </button>
                            </span>
                        </div>
